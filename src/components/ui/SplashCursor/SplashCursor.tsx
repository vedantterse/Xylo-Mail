...
export default function SplashCursor({
  ...
  VELOCITY_DISSIPATION = 4, // Increased for faster fade
  PRESSURE = 0.1,
  PRESSURE_ITERATIONS = 20,
  CURL = 3,
  SPLAT_RADIUS = 0.3,      // Increased for better visibility
  SPLAT_FORCE = 4000,      // Reduced for smoother trailing effect
  SHADING = true,
  ...
  useEffect(() => {
    ...
    function generateColor(): ColorRGB {
      // Indian flag colors with slight adjustments for better visual effect
      const colors = [
        { r: 1.0, g: 0.6, b: 0.0 },  // Saffron
        { r: 0.9, g: 0.9, b: 0.9 },  // White
        { r: 0.0, g: 0.0, b: 0.6 },  // Navy Blue (Ashoka Chakra)
        { r: 0.0, g: 0.5, b: 0.0 },  // Green
      ];

      // Cycle through colors based on time
      const now = Date.now();
      const index = Math.floor((now % 2000) / 500) % colors.length; // Ensure index stays within bounds
      const color = colors[index] || colors[0]; // Fallback to first color if undefined

      // Reduce intensity for better visual blend
      return {
        r: color.r * 0.15,
        g: color.g * 0.15,
        b: color.b * 0.15
